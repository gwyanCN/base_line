...
class EpisodicBatchSampler(data.Sampler):
    def __init__(self, labels,masks, n_episodes, n_way, n_samples):
        ...
        # if self.n_way > len(self.samples_indices.keys()):
    ...
    def __iter__(self):
        for batch in range(self.n_episodes):
            batch = []
            classes = [self.class_set[i] for i in torch.randperm(len(self.samples_indices.keys()))[:self.n_way].tolist()] # torch.randperm(n)返回一个0到n-1的数组
            for c in classes:
