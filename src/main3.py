...
def train_protonet2(model,train_loader,valid_loader,conf):
    ...
    time_start=time.time()
    for epoch in range(num_epochs):
        trainer.do_epoch2(epoch=epoch,scheduler=lr_scheduler,disable_tqdm=disable_tqdm,model=model,alpha=alpha,optimizer=optim,aug=aug)
        # Evaluation on validation set
...
def main(conf : DictConfig):
    ...
    if conf.set.features:
        ...
        # Num_extract_train,data_shape = feature_transform(conf=conf,mode="train",aug=aug) # train data
    ...
    if conf.set.train: # train
        ...
        if meta_learning:
            ...
            if conf.train.train_data:
                ...
                torch.save(state,'/media/b227/ygw/Dcase2023/baseline/src/train_data/datasets.pth')
            ...
            train_dataset2 = torch.utils.data.TensorDataset(X_tr,Y2_tr,Y_tr) # 利用torch 的 dataset,整合X,Y
    ...
    if conf.set.eval: # eval
        ...
        # model = nn.DataParallel(model,device_ids=device_ids)
        student_model = get_model('TSVAD1',conf.train.num_classes,ema=True).cuda()
        ...
        for i in range(2*TOTAL_LENGTH//4,3*TOTAL_LENGTH//4):
            ...
            prob_middle = flatten_res_melt(prob_thresh,hop_seg,int(nframe-start_index_query))
        ...
        csv_path_ori = os.path.join(conf.path.work_path,'src','output_csv','ori','Eval_out_ori_3.csv')
        ...
        csv_path_tim = os.path.join(conf.path.work_path,'src','output_csv','tim','Eval_out_tim_3.csv')
